...
```bash
docker build --tag=nullptr:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=nullptr:latest --build-arg USE_CLANG=1 .
```
